...
import ImageWaterfall from '../../../../components/ImageWaterfall/imageWaterfall'
...
let param = {
  ...
  requestImages() {
    ...
    var keyWord = this.data.queryWord //关键字
    ...
    let url =
      'https://image.baidu.com/search/acjson?tn=resultjson_com&ipn=rj&ct=201326592&is=&fp=result&cl=2&lm=-1&ie=utf-8&oe=utf-8&adpicid=&st=-1&z=&ic=0&s=&se=&tab=&width=&height=&face=0&istype=2&qc=&nc=1&fr=&logid=7578759849659843688&cg=girl&hd=&latest=&copyright=&expermode=&nojc=&isAsync='
    app.promise(url, requestData).then(
      (res) => {
        let responseData = []
        try {
  ...
  _lookBigImg(currentSrc) {
    // let currentSrc = e.detail.currentTarget.dataset.src
    if (isToImageDetail) {
      //是否前往图片详情页查看图片
      Taro.navigateTo({
        url:
          '../imageDetail/imageDetail?bigImgSrc=' +
...
class _C extends React.Component {
  onShareTimeline(obj) { // 分享朋友圈
    console.log('onShareTimeline', obj)
    return {
      title: this.data.queryWord,
      // query:'',
      // imageUrl: this.state.imageList[0]
    }
  }

  onShareAppMessage(res) { // 发送为朋友
    if (res.from === 'button') {
      // 来自页面内转发按钮
  ...
  render() {
    const { columns, imageList } = this.data
    return (
      <View className="container">
        <ImageWaterfall
          onLoaded={this._loaded}
          onLookBigImg={this._lookBigImg}
          columns={columns}
          imageList={[...imageList]}/>
      </View>
    )
